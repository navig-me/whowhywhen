...
from app.models.user import User, UserAlertNotification
from app.schemas.user import ChangePasswordForm, UserCreate, UserRead, UserStatusRead
from app.services.email_service import send_password_reset_email
from app.services.fa_service import (
...
def forgot_password(email: str, session: Session = Depends(get_session)):
    ...
    temp_password = pyotp.random_base32()[
        :8
...
def get_2fa_qr_code(
    ...
    session.commit()
    return {
        "totp_uri": generate_totp_uri(totp_secret, user.email, issuer_name="WhoWhyWhen")
    }
...
def read_users_me(
    ...
    for project in user_projects:
        user_request_count += (
            ...
            .filter(APILog.user_project_id == project.id)
            .count()
        )

    user_project_ids = [project.id for project in user_projects]
    unread_alert_count = (
        session.query(UserAlertNotification)
        .filter(UserAlertNotification.user_project_id.in_(user_project_ids))
        .filter(UserAlertNotification.read_at == None)
        .count()
    )
